...
const mahasiswaPricingPlans = [
  {
    plan: 'Mahasiswa Dasar',
    price: 'Rp 1.5jt',
    features: [
      '6 Sesi/Bulan',
      'Materi Full-Stack atau AI Dasar',
      'Akses Komunitas Mahasiswa',
      'Laporan Kemajuan',
    ],
    isFeatured: false,
  },
...
const PricingCard = ({ plan, price, features, isFeatured }) => {
  const cardClasses = `relative p-6 sm:p-8 rounded-3xl border border-light-text/10 dark:border-dark-text/10 transition-all duration-300 transform hover:-translate-y-2 hover:shadow-xl ${
    isFeatured
      ? 'bg-gradient-to-br from-purple-200/20 to-purple-300/40 dark:from-purple-800/30 dark:to-purple-900/50 text-white scale-[1.03] shadow-lg'
          ...
          >
            <CheckCircleIcon
              className={`h-4 w-4 sm:h-5 sm:w-5 flex-shrink-0 ${
                isFeatured ? 'text-accent' : 'text-purple-500'
              }`}
              aria-hidden="true"
            />
            <span>{feature}</span>
